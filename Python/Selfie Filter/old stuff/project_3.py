...
def buck_teeth_filter(video):
    """
    This method applies the buck teeth filter onto the video.
    It draws rectangles in the corresponding places of interest for this filter.
    In this case, it is the face and mouth, we include the eyes because the
    mouth has a relationship with the eyes.
    
    The objective is to take an image of buck teeth and place it on the mouth
    ...
    #Draw a rectangle around the faces(Green)
       for (x, y, w, h) in faces:
        ...
        #Draws the rectangle around the eyes(Blue)
          for (ex, ey, ew, eh) in eyes:
             cv2.rectangle(roi_color, (ex, ey), (ex+ew, ey+eh), (255, 0, 0), 2)
        #Mouth Rectangle(Purple)
          cv2.rectangle(frame, (int(x+(w/3)), int(y+((2/3)*h))), (int((x+((2/3)*w))), int((y+((2/3)*h))+eh)),(255, 0, 255), 2)
       # Display the resulting frame
       cv2.imshow('BuckTeeth', frame)
    #Press the 'q' key to close the video feed
       if cv2.waitKey(1) & 0xFF == ord('q'):
          break
