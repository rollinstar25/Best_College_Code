...
def dog_face_filter(video, record = False):
    """
    ...
    if record == True:
        ...
        out = cv2.VideoWriter('output_dog_face.avi',fourcc, 20.0, (640,480))
...
def heart_eyes_filter(video, record = False):
    """
    ...
    if record == True:
        fourcc = cv2.VideoWriter_fourcc(*'XVID')
        out = cv2.VideoWriter('output_heart_eyes.avi',fourcc, 20.0, (640,480))
...
def mustache_filter(video, record = False):
    """
    ...
    if record == True:
        fourcc = cv2.VideoWriter_fourcc(*'XVID')
        out = cv2.VideoWriter('output_mustache.avi',fourcc, 20.0, (640,480))
...
def hat_filter(video, record = False):
    """
    This method applies a mustache onto the video.
    It draws rectangles in the corresponding place of interest for this filter.
    This filter approached the problem a little different in that it
    ...
    if record == True:
        fourcc = cv2.VideoWriter_fourcc(*'XVID')
        out = cv2.VideoWriter('output_hat.avi',fourcc, 20.0, (640,480))
...
def hat_and_stache_filter(video, record = False):
    hat = cv2.imread('hat_filter.png')
    stache = cv2.imread('mustache_filter.png')
    
    if record == True:
        fourcc = cv2.VideoWriter_fourcc(*'XVID')
        out = cv2.VideoWriter('output_hat_and_stache.avi',fourcc, 20.0, (640,480))
    while True:
        # Capture frame-by-frame
        ret, frame = capture.read()
...
def face_swap(video, record = False):
    """
    ...
    if record == True:
        fourcc = cv2.VideoWriter_fourcc(*'XVID')
        out = cv2.VideoWriter('output_face_swap.avi',fourcc, 20.0, (640,480))
...
#Filters with video capture
#face_swap(capture,True)
dog_face_filter(capture, True)
#mustache_filter(capture, True)
#hat_filter(capture, True)
#heart_eyes_filter(capture, True)
#hat_and_stache_filter(capture)   NOT WORKING
